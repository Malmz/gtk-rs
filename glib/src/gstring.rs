...
mod tests {
    ...
    fn test_from_glib_container() {
        unsafe {
            let a: GString = FromGlibContainer::from_glib_container_num(
                ffi::g_strdup("hello_world".as_ptr() as *const i8),
                5,
            );
            assert_eq!("hello", a.as_str());
            let b: GString = FromGlibContainer::from_glib_none_num("hello_world".as_ptr(), 5);
            assert_eq!("hello", b.as_str());

            let c: GString =
                FromGlibContainer::from_glib_none_num(std::ptr::null() as *const i8, 0);
            assert_eq!("", c.as_str());

            let d: GString = FromGlibContainer::from_glib_none_num("".as_ptr(), 0);
            assert_eq!("", d.as_str());

            let e: GString = FromGlibContainer::from_glib_container_num(
                ffi::g_strdup(std::ptr::null() as *const i8),
                0,
            );
            assert_eq!("", e.as_str());
        }
